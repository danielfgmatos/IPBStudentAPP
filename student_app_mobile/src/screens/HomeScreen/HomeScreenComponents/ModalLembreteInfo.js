...
const item = values.item;
  ...
  const updateLembrete = async (values) => {
    ...
    await api
        .get(
            `/lembrete/update-lembrete/?id_lembrete=${values.id_lembrete}&codigo_disciplina_fk=${values.codigo_disciplina_fk}&tipo=${values.tipo}&data_inicio=${values.data_inicio}&data_vencimento=${values.data_vencimento}&calendar_date=${values.data_vencimento}&descricao=${values.descricao}&numero_mecanografico_fk=${values.numero_mecanografico_fk}`,
            { headers: { Authorization: `Bearer ${userContext[0].token}` } }
        )
        .then((response) => {
          setUpdate(false);
            ...
            { editar ?
                ...
                <View style={styles.modalContainer}>
                  <View style={styles.modalHeadersView}>
                    ...
                    <Text style={styles.modalData}>{item.descricao}</Text>
                  </View>

                  <View style={styles.modalHeadersView}>
                    <Text style={styles.modalHeaders}>Criado:</Text>
                    <Text style={styles.modalData}>{item.data_inicio}</Text>
                  </View>

                  <View style={styles.modalHeadersView}>
                    <Text style={styles.modalHeaders}>Data de Vencimento:</Text>
                    <Text style={styles.modalData}>{item.data_vencimento}</Text>
                  </View>
                  ...
                  <View  style={{ flexDirection: "row", justifyContent: "space-between" }}>
                    ...
                    <ModalButton
                        ...
                        onPress={() => setModalOpen(false)}
                    />
                    <TouchableOpacity onPress={() => removerLembrete(item)}>
                      <View style={styles.buttonSubmit}>
